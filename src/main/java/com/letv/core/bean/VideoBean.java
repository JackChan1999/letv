...
public class VideoBean implements LetvBaseBean {
    ...
    public void saveLoadingPic() {
        if (!PreferencesManager.getInstance().loadingPicNotSaved(this.pid + "", this.vid + "")) {
            if (!TextUtils.isEmpty(this.poster960_540) || !TextUtils.isEmpty(this.poster1080_608)) {
                String backgroundUrl = TextUtils.isEmpty(this.poster960_540) ? this.poster1080_608 : this.poster960_540;
                if (isFeature() && (this.cid == 2 || this.cid == 1017 || this.cid == 5 || this.cid == 1)) {
                    PreferencesManager.getInstance().setLoadingBackground(this.pid + "", backgroundUrl);
                } else {
                    PreferencesManager.getInstance().setLoadingBackground(this.vid + "", backgroundUrl);
                }
                LogInfo.log("zhaosumin", "VideoPlayerParser backgroundUrl == " + backgroundUrl);
                ImageDownloader.getInstance().download(backgroundUrl);
            }
        }
    }
